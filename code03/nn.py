...
def getTotalVector(data, name):
    ...
    model = gensim.models.Word2Vec.load(namepath + name + 'gensimModel-Author.model')
    print(4)
    ...
    for i in range(1, len(l)):
        model = gensim.models.Word2Vec.load(namepath + name + 'gensimModel-'+l[i]+'.model')
        row = len(model.wv.vocab)
        Mat = []
        for i in range(row):
...
def getContrast(datacontrast):

    contrastFileData = datacontrast.split('\n')[:-1]
    contrast = []
    p = 0
    count = 0
...
def getSamples(datacontrast, data, name, num=10):

    global rstToWrite
    totalVectors = getTotalVector(data,name)
    contrast = getContrast(datacontrast)
    manyWeis = getWeiIndexs(contrast, num)
    print(manyWeis)
    length = len(manyWeis)
    # print(length)
    manyArticles = getArticleIndexs(manyWeis, contrast)
    f = open(path+"/"+name+"/"+name+'-manyArticles-' + str(num) + '.pkl', 'wb')
